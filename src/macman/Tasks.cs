...
namespace fmcl
{
    public class Tasks
    {
        ...
        {
            JObject o = await TwitchAPI.TryGetVersionFileAsync(id, version);
        ...
        public static async Task DownloadMcmod(JObject o, string version, string path)
        {
            ...
            foreach (var de in o["dependencies"])
                if (de["type"].Value<int>() == 3)
                    DownloadMcmod(await TwitchAPI.TryGetVersionFileAsync(de["addonId"].Value<string>(), version), version,
                        path);
        }
